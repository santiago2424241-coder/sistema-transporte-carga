...
Versión 3.3 - Conectado a Supabase (PostgreSQL)
Contexto: Colombia
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        # Tabla de viajes con nuevos campos actualizados
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS viajes (
                ...
                es_urbano INTEGER NOT NULL,  -- NUEVO
                ...
                total_gastos REAL,
                legalizacion REAL,
                saldo_anticipo REAL,        -- NUEVO (Formula Anticipo - Legalizacion)
        ...
        # Tabla de rutas con campo urbano
        cursor.execute('''
    ...
    def guardar_viaje(self, calculadora, observaciones=""):
        ...
        cursor.execute('''
            INSERT INTO viajes (
                ...
                distancia_km, dias_viaje, es_frontera, es_urbano, hubo_parqueo,
                nomina_admin, nomina_conductor, comision_conductor,
                mantenimiento, seguros, tecnomecanica, llantas, aceite,
                combustible, galones_necesarios, flypass, peajes,
                cruce_frontera, hotel, comida, parqueo,
                cargue_descargue, otros, anticipo, total_gastos, legalizacion,
                saldo_anticipo, punto_equilibrio, valor_flete, utilidad, rentabilidad,
        ...
        ''', (
            ...
            # AQUI HAY CONVERTIDORES int() para las columnas viejas que son 0/1
            int(calculadora.es_frontera),
            calculadora.es_urbano,       # Nueva columna es Boolean
            int(calculadora.hubo_parqueo),# Columna vieja es Entero
            costos['nomina_admin'],
    ...
    def obtener_dashboard_data(self):
        ...
        cursor.execute("""
            SELECT
                ...
                SUM(valor_flete) as total_ingresos,
        ...
        cursor.execute("""
            SELECT placa, COUNT(*) as viajes, SUM(total_gastos) as gastos, SUM(valor_flete) as ingresos, SUM(utilidad) as utilidad
            FROM viajes WHERE to_date(fecha_creacion, 'YYYY-MM-DD') >= %s GROUP BY placa ORDER BY utilidad DESC
        """, (inicio_mes,))
        data['por_tractomula'] = cursor.fetchall()
       
        cursor.execute("""
            SELECT conductor, COUNT(*) as viajes, SUM(utilidad) as utilidad, AVG(utilidad) as utilidad_promedio
            FROM viajes WHERE to_date(fecha_creacion, 'YYYY-MM-DD') >= %s GROUP BY conductor ORDER BY utilidad DESC
        """, (inicio_mes,))
        data['por_conductor'] = cursor.fetchall()
       
        cursor.execute("""
            SELECT origen, destino, COUNT(*) as viajes, AVG(utilidad) as utilidad_promedio, SUM(utilidad) as utilidad_total
            FROM viajes WHERE to_date(fecha_creacion, 'YYYY-MM-DD') >= %s GROUP BY origen, destino ORDER BY utilidad_total DESC LIMIT 5
        """, (inicio_mes,))
        data['rutas_rentables'] = cursor.fetchall()
       
        cursor.execute("""
            SELECT to_char(to_date(fecha_creacion, 'YYYY-MM-DD'), 'YYYY-MM') as mes, COUNT(*) as viajes, SUM(total_gastos) as gastos, SUM(valor_flete) as ingresos, SUM(utilidad) as utilidad
            FROM viajes WHERE to_date(fecha_creacion, 'YYYY-MM-DD') >= CURRENT_DATE - INTERVAL '6 months' GROUP BY mes ORDER BY mes
        """)
    ...
    def obtener_totales_por_placa(self, fecha_inicio=None, fecha_fin=None):
        ...
        query = """
            SELECT 
                placa,
                SUM(valor_flete) as total_cxc,
                SUM(nomina_admin) as total_admin,
                SUM(nomina_conductor) as total_parafiscales,
                SUM(comision_conductor) as total_comision,
                SUM(mantenimiento) as total_mantenimiento,
                SUM(seguros) as total_seguros,
                SUM(tecnomecanica) as total_tecnomecanica,
                SUM(llantas) as total_llantas,
                SUM(aceite) as total_aceite,
                SUM(combustible) as total_combustible,
                SUM(flypass) as total_flypass,
                SUM(peajes) as total_peajes,
                SUM(cruce_frontera) as total_cruce_frontera,
                SUM(hotel) as total_hotel,
                SUM(comida) as total_comida,
                SUM(parqueo) as total_parqueo,
                SUM(cargue_descargue) as total_cargue_descargue,
                SUM(otros) as total_otros,
                SUM(anticipo) as total_anticipo,
                SUM(legalizacion) as total_legalizacion,
                SUM(saldo_anticipo) as total_saldo_anticipo,
                SUM(ant_empresa) as total_ant_empresa,
        ...
        if not df.empty:
            ...
            # df['saldo_total'] era 'total_saldo_empresa', dejamos la columna calculada existente
            df['saldo_total'] = df['total_saldo_empresa'] 
    ...
    def guardar_ruta(self, ruta: 'Ruta'):
        conn = self.get_connection()
        cursor = conn.cursor()
        # es_frontera se convierte a int (1/0), es_urbano queda como boolean (True/False)
        cursor.execute('''
            INSERT INTO rutas (origen, destino, distancia_km, es_frontera, es_urbano)
            VALUES (%s, %s, %s, %s, %s)
        ''', (ruta.origen, ruta.destino, ruta.distancia_km, int(ruta.es_frontera), ruta.es_urbano))
        conn.commit()
    ...
    def obtener_rutas(self):
        ...
        for row in cursor.fetchall():
            rutas.append(Ruta(origen=row[1], destino=row[2], distancia_km=row[3], 
                               es_frontera=bool(row[4]), es_urbano=bool(row[5])))
        conn.close()
        return rutas
...
class DatosColombia:
    ...
    COMISION_URBANO = 180000         # Nueva constante
    ...
    SEGURO_2 = 6000000
    SEGURO_3 = 16000000
    TECNOMECANICA_ANUAL = 460000     # Actualizado
    
    LLANTAS_COSTO = 1300000         # Actualizado
    LLANTAS_CANTIDAD = 22
    LLANTAS_KM = 80000
...
PLACA_CONDUCTOR = {
    "NOX459": "HABID CAMACHO", "NOX460": "JOSE ORTEGA PEREZ", "NOX461": "ISAAC TAFUR",
    "SON047": "ISAIAS VESGA", "SON048": "FLAVIO ROSENDO MALTE TUTALCHA",
    "SOP148": "SLITH JOSE ORTEGA PACHECO", "SOP149": "ABRAHAM SEGUNDO ALVAREZ VALLE",
    "SOP150": "RAMON TAFUR HERNANDEZ", "SRO661": "", "SRO672": "PEDRO VILLAMIL",
    "TMW882": "JESUS DAVID MONTE MOSQUERA", "TRL282": "CHRISTIAN MARTINEZ NAVARRO",
    "TRL298": "YEIMI DUQUE ZULUAGA", "UYQ308": "JULIAN CALETH CORONADO",
    "UYV084": "CARLOS TAFUR", "UYY788": "EDUARDO RAFAEL OLIVARES ALCAZAR",
}

# ==================== CALCULADORA DE COSTOS ====================
class CalculadoraCostos:
    """Calcula todos los costos del viaje con fórmulas reales"""
   
    def __init__(self, tractomula: Tractomula, conductor: Conductor, ruta: Ruta,
                 dias_viaje: int, es_frontera: bool, es_urbano: bool, hubo_parqueo: bool,
                 flypass: float, peajes: float, hotel: float, comida: float,
                 cargue_descargue: float, otros: float, anticipo: float,
                 hubo_anticipo_empresa: bool,
                 valor_flete: float, datos: DatosColombia):
        self.tractomula = tractomula
        self.conductor = conductor
        self.ruta = ruta
        self.dias_viaje = dias_viaje
        self.es_frontera = es_frontera
        self.es_urbano = es_urbano
    ...
    def calcular_comision_conductor(self) -> float:
        """Lógica Urbano > Frontera > Nacional"""
        if self.es_urbano:
            return self.datos.COMISION_URBANO
        else:
            return self.datos.COMISION_FRONTERA if self.es_frontera else self.datos.COMISION_NO_FRONTERA
    ...
    def calcular_seguros(self) -> float:
        seguro_diario = ((self.datos.SEGURO_1 / 365) + (self.datos.SEGURO_2 / 365) + (self.datos.SEGURO_3 / 14 / 365))
    ...
    def calcular_galones_necesarios(self) -> float:
        if self.tractomula.consumo_km_galon <= 0: return 0.0
    ...
    def calcular_costos_totales(self) -> Dict[str, float]:
        ...
        # Anticipo Empresa (Condicional)
        ant_empresa = self.valor_flete * self.datos.MARGEN_ANT_EMPRESA if self.hubo_anticipo_empresa else 0.0
        saldo_empresa = self.valor_flete - ant_empresa
        
        # Nuevo Saldo (Anticipo - Legalizacion)
        saldo_anticipo = self.anticipo - legalizacion
       
        return {
            'nomina_admin': round(nomina_admin, 2), 'nomina_conductor': round(nomina_conductor, 2),
            'comision_conductor': round(comision_conductor, 2), 'mantenimiento': round(mantenimiento, 2),
            'seguros': round(seguros, 2), 'tecnomecanica': round(tecnomecanica, 2),
            'llantas': round(llantas, 2), 'aceite': round(aceite, 2),
            'combustible': round(combustible, 2), 'galones_necesarios': round(galones_necesarios, 2),
            'cruce_frontera': round(cruce_frontera, 2), 'parqueo': round(parqueo, 2),
            'total_gastos': round(total_gastos, 2), 'legalizacion': round(legalizacion, 2),
            'anticipo': round(self.anticipo, 2), 'saldo_anticipo': round(saldo_anticipo, 2),
            'punto_equilibrio': round(punto_equilibrio, 2), 'utilidad': round(utilidad, 2),
            'rentabilidad': round(rentabilidad, 2), 'ant_empresa': round(ant_empresa, 2),
...
Es urbano: {'Sí' if calculadora.es_urbano else 'No'}
...
ANTICIPO: ${formatear_numero(costos['anticipo']):>18} COP
{'-'*70}
SALDO (ANTICIPO - LEGALIZACIÓN): ${formatear_numero(costos['saldo_anticipo']):>18} COP
{'='*70}
...
"""
    ...
    def generar_excel(calculadoras: List[CalculadoraCostos]) -> io.BytesIO:
        ...
        row = 4
        headers = ['Ruta', 'Placa', 'Conductor', 'Distancia (km)', 'Días', 'Galones', 'Combustible', 'Total Gastos', 'Valor Flete', 'Utilidad', 'Rentabilidad %', 'Punto Equilibrio']
        for col, header in enumerate(headers, start=1):
            c = ws_resumen.cell(row=row, column=col)
            c.value, c.font, c.fill, c.alignment, c.border = header, header_font, header_fill, Alignment(horizontal='center', vertical='center'), border
       
        row = 5
        for calc in calculadoras:
            costos = calc.calcular_costos_totales()
            datos = [f"{calc.ruta.origen} → {calc.ruta.destino}", calc.tractomula.placa, calc.conductor.nombre, calc.ruta.distancia_km, calc.dias_viaje, costos['galones_necesarios'], costos['combustible'], costos['total_gastos'], calc.valor_flete, costos['utilidad'], costos['rentabilidad'], costos['punto_equilibrio']]
            for col, valor in enumerate(datos, start=1):
                c = ws_resumen.cell(row=row, column=col)
                c.value, c.alignment, c.border = valor, Alignment(horizontal='center' if col <= 3 else 'right'), border
                if col == 4: c.number_format = '#,##0'
                elif col == 5: c.number_format = '#,##0'
                elif col == 6: c.number_format = '#,##0.00'
                elif 7 <= col <= 10: c.number_format = '$#,##0'
                elif col == 11: c.number_format = '#,##0.0"%"'
                elif col == 12: c.number_format = '$#,##0'
            row += 1
        ...
        for idx, calc in enumerate(calculadoras, start=1):
            ...
            ws.merge_cells('A1:D1'); ws['A1'] = f"{calc.ruta.origen} → {calc.ruta.destino}"
            ws['A1'].font = Font(size=14, bold=True, color="1F4E78"); ws['A1'].alignment = Alignment(horizontal='center')
            
            r, detalles_costos = 3, [('1. Nómina Admin', costos['nomina_admin']), ('2. Nómina Conductor', costos['nomina_conductor']), ('3. Comisión Conductor', costos['comision_conductor']), ('4. Mantenimiento', costos['mantenimiento']), ('5. Seguros', costos['seguros']), ('6. Tecnomecánica', costos['tecnomecanica']), ('7. Llantas', costos['llantas']), ('8. Aceite', costos['aceite']), ('9. Combustible', costos['combustible']), ('10. Flypass', calc.flypass), ('11. Peajes', calc.peajes), ('12. Cruce Frontera', costos['cruce_frontera']), ('13. Hotel', calc.hotel), ('14. Comida', calc.comida), ('15. Parqueo', costos['parqueo']), ('16. Cargue/Descargue', calc.cargue_descargue), ('17. Otros', calc.otros)]
            
            ws.merge_cells(f'A{r}:D{r}'); c = ws[f'A{r}']; c.value, c.font, c.fill = "DESGLOSE DE COSTOS", subheader_font, subheader_fill; r += 1
            ws[f'A{r}'], ws[f'B{r}'] = "Concepto", "Monto (COP)"; ws[f'A{r}'].font, ws[f'B{r}'].font = Font(bold=True), Font(bold=True); r += 1
            
            for concepto, monto in detalles_costos:
                ws[f'A{r}'], ws[f'B{r}'] = concepto, monto; ws[f'B{r}'].number_format = '$#,##0'; r += 1
            r += 1
            
            ws.merge_cells(f'A{r}:D{r}'); c = ws[f'A{r}']; c.value, c.font, c.fill = "RESULTADOS", subheader_font, subheader_fill; r += 1
            
            resultados = [('TOTAL GASTOS', costos['total_gastos']), ('LEGALIZACIÓN', costos['legalizacion']), ('ANTICIPO', costos['anticipo']), ('SALDO (Anticipo - Legalización)', costos['saldo_anticipo']), ('', ''), ('VALOR DEL FLETE', calc.valor_flete), ('UTILIDAD (UT)', costos['utilidad']), ('RENTABILIDAD (%)', costos['rentabilidad']), ('', ''), ('ANT. EMPRESA (90%)', costos['ant_empresa']), ('SALDO EMPRESA', costos['saldo_empresa'])]
            for label, value in resultados:
                if label:
                    ws[f'A{r}'], ws[f'B{r}'] = label, value; ws[f'A{r}'].font = Font(bold=True)
                    if 'TOTAL' in label or 'UTILIDAD' in label: ws[f'A{r}'].fill, ws[f'B{r}'].fill, ws[f'B{r}'].font = total_fill, total_fill, total_font
                    if label == 'RENTABILIDAD (%)': ws[f'B{r}'].number_format = '#,##0.0"%"'
                    else: ws[f'B{r}'].number_format = '$#,##0'
                r += 1
            ws.column_dimensions['A'].width = 30; ws.column_dimensions['B'].width = 20; ws.column_dimensions['C'].width = 15; ws.column_dimensions['D'].width = 15
...
def mantener_app_activa():
    ...
    if tiempo_inactivo.total_seconds() > 240:
        st.session_state.ultima_actividad = datetime.now(); st.rerun()
    with st.sidebar:
        st.markdown("---"); minutos = int(tiempo_inactivo.total_seconds()) // 60; segundos = int(tiempo_inactivo.total_seconds()) % 60
        st.caption(f"⏱️ Sesión activa: {minutos}m {segundos}s")
        if st.button("🔄 Refrescar", key="refresh_manual"): st.session_state.ultima_actividad = datetime.now(); st.rerun()
...
def main():
    ...
    datos, db = st.session_state.datos, st.session_state.db
    ...
    with st.sidebar:
        ...
        st.info(f"**Nóminas:** - Admin base: ${formatear_numero(datos.NOMINA_ADMIN_BASE)} - Conductor/día: ${formatear_numero(datos.NOMINA_CONDUCTOR_DIA)} **Comisiones:** - Frontera: ${formatear_numero(datos.COMISION_FRONTERA)} - No frontera: ${formatear_numero(datos.COMISION_NO_FRONTERA)} **Otros:** - Cruce frontera: ${formatear_numero(datos.CRUCE_FRONTERA)} - Parqueo/día: ${formatear_numero(datos.PARQUEO_DIA)}")
   
    tab0, tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Dashboard", "1. Tractomulas", "2. Rutas", "3. Conductores", "4. Cálculo de Viaje", "5. Reportes", "6. 📂 Trazabilidad", "7. Acumulado por Flota"])
   
    with tab0:
        st.header("📊 Dashboard - Resumen de tu Negocio")
        dashboard_data = db.obtener_dashboard_data(); mes_actual = dashboard_data['mes_actual']
        st.subheader(f"📅 Resumen del Mes - {datetime.now().strftime('%B %Y')}")
        col1, col2, col3, col4, col5 = st.columns(5)
        
        with col1: st.metric("📊 UT BRUTA", f"${formatear_numero(mes_actual['total_ingresos'])}", help="Suma total de Cuentas por Cobrar (Valor Flete)")
        with col2: st.metric("📈 UT NETA", f"${formatear_numero(mes_actual['total_utilidad'])}")
        with col3: st.metric("💸 Gastos Totales", f"${formatear_numero(mes_actual['total_gastos'])}")
        with col4: st.metric("🚛 Viajes Realizados", f"{mes_actual['total_viajes']}")
        with col5: st.metric("🛣️ Kilómetros", f"{formatear_numero(mes_actual['total_km'])} km")
        
        if mes_actual['total_viajes'] > 0:
            col1, col2, col3 = st.columns(3)
            with col1:
                if mes_actual['total_ingresos'] > 0:
                    porcentaje_ut = (mes_actual['total_utilidad'] / mes_actual['total_ingresos']) * 100
                    color = "🟢" if porcentaje_ut >= 20 else "🟡" if porcentaje_ut >= 10 else "🔴"
                    st.info(f"{color} **% UT (Rentabilidad):** {porcentaje_ut:.2f}%")
        ...
        if dashboard_data['por_tractomula']:
            ...
            for col in ['Gastos', 'Ingresos', 'Utilidad']: tractomula_df[col] = tractomula_df[col].apply(lambda x: f"${formatear_numero(x)}")
            st.dataframe(tractomula_df, use_container_width=True, hide_index=True)
        else: st.info("No hay datos de tractomulas este mes")
        ...
        if placa_seleccionada:
            ...
            if not df_placa.empty:
                ...
                with col3:
                    ...
                    st.metric("Saldo Total", f"${formatear_numero(row['saldo_total'])}")
            else: st.info("No hay datos para esta placa")

        st.subheader("Comparativa entre Unidades")
        df_totales = db.obtener_totales_por_placa()
        if not df_totales.empty:
            fig_utilidad = px.bar(df_totales, x='placa', y='total_ut', title="Utilidad Total por Unidad"); st.plotly_chart(fig_utilidad)
            fig_gastos = px.bar(df_totales, x='placa', y='total_gastos', title="Gastos Totales por Unidad"); st.plotly_chart(fig_gastos)
        else: st.info("No hay datos para comparar")
        ...
        with st.form(key="form_tractomula"):
            ...
            if submit and placa:
                ...
                if db.guardar_tractomula(tractomula):
                    ...
                    st.success(f"✅ Tractomula {placa} guardada!"); st.rerun()
            ...
            for idx, t in enumerate(st.session_state.tractomulas):
                ...
                with col2:
                    if st.button("🗑️", key=f"eliminar_tractomula_{idx}"):
                        db.eliminar_tractomula(t.placa); st.session_state.tractomulas = db.obtener_tractomulas()
                        st.success(f"Tractomula {t.placa} eliminada"); st.rerun()
    ...
    with tab2:
        ...
        with st.form(key="form_ruta"):
            ...
            with col2:
                st.subheader("Tipo de Ruta")
                es_urbano = st.checkbox("¿Es viaje urbano?", help="El viaje es dentro de la ciudad (Comisión $180k)")
                es_frontera = st.checkbox("¿Es ruta a frontera?", help="El viaje es hacia frontera (Comisión $500k)")
                if es_urbano and es_frontera: st.warning("Nota: Si es Urbano, la comisión será de $180k (prioridad).")
            ...
            if submit and origen and destino:
                ...
                ruta = Ruta(origen, destino, distancia_km, es_frontera, es_urbano)
                ruta_id = db.guardar_ruta(ruta); st.session_state.rutas = db.obtener_rutas()
                st.success(f"✅ Ruta {origen} → {destino} guardada!"); st.rerun()
       
        if st.session_state.rutas:
            st.subheader("Rutas Registradas")
            conn = db.get_connection(); cursor = conn.cursor()
            cursor.execute("SELECT id, origen, destino, distancia_km, es_frontera, es_urbano FROM rutas ORDER BY origen, destino")
            rutas_con_id = cursor.fetchall(); conn.close()
            for ruta_data in rutas_con_id:
                ruta_id, origen, destino, dist, es_front, es_urb = ruta_data
                col1, col2 = st.columns([4, 1])
                with col1:
                    frontera_tag = "🌐 FRONTERA" if es_front else ""
                    urbano_tag = "🏙️ URBANO" if es_urb else ""
                    st.write(f"**{origen}** → **{destino}** ({formatear_numero(dist)} km) {urbano_tag} {frontera_tag}")
                with col2:
                    if st.button("🗑️", key=f"eliminar_ruta_{ruta_id}"):
                        db.eliminar_ruta(ruta_id); st.session_state.rutas = db.obtener_rutas()
                        st.success("Ruta eliminada"); st.rerun()
   
    with tab3:
        st.header("Tus Conductores")
        if 'conductores_cedulas' not in st.session_state:
            st.session_state.conductores_cedulas = {
                "HABID CAMACHO": "123456789", "JOSE ORTEGA PEREZ": "987654321", "ISAAC TAFUR": "456789123",
                "ISAIAS VESGA": "789123456", "FLAVIO ROSENDO MALTE TUTALCHA": "321654987",
                "SLITH JOSE ORTEGA PACHECO": "654987321", "ABRAHAM SEGUNDO ALVAREZ VALLE": "147258369",
                "RAMON TAFUR HERNANDEZ": "369258147", "PEDRO VILLAMIL": "258369147",
                "JESUS DAVID MONTE MOSQUERA": "951753486", "CHRISTIAN MARTINEZ NAVARRO": "486159753",
                "YEIMI DUQUE ZULUAGA": "753486159", "JULIAN CALETH CORONADO": "159753486",
                "CARLOS TAFUR": "357159486", "EDUARDO RAFAEL OLIVARES ALCAZAR": "486357159"
            }
        with st.form(key="form_conductor"):
            ...
            with col1:
                ...
                if nombre_seleccion == '(Escribir nuevo)':
                    nombre = st.text_input("Nombre manual"); cedula_auto = ""
                else:
                    nombre = nombre_seleccion; cedula_auto = st.session_state.conductores_cedulas.get(nombre, "")
            with col2:
                if cedula_auto:
                    cedula = st.text_input("Cédula", value=cedula_auto); st.info("📋 Cédula encontrada automáticamente")
                else:
            ...
            if submit and nombre and cedula:
                ...
                if db.guardar_conductor(conductor):
                    ...
                    st.success(f"✅ Conductor {nombre} guardado!"); st.rerun()
            ...
            for idx, c in enumerate(st.session_state.conductores):
                ...
                with col2:
                    if st.button("🗑️", key=f"eliminar_conductor_{idx}"):
                        db.eliminar_conductor(c.nombre); st.session_state.conductores = db.obtener_conductores()
                        st.success(f"Conductor {c.nombre} eliminado"); st.rerun()
    ...
    with tab4:
        ...
        if not (st.session_state.tractomulas and st.session_state.conductores and st.session_state.rutas):
            ...
            with st.form(key="form_calculo"):
                ...
                st.divider(); st.subheader("📊 Parámetros del Viaje")
                col1, col2, col3 = st.columns(3)
                with col1:
                    # Checkbox de Urbano y Frontera
                    es_urbano = st.checkbox("¿Es viaje urbano?", value=ruta_obj.es_urbano, help="Comisión fija de $180.000")
                    es_frontera = st.checkbox("¿Es viaje a frontera?", value=ruta_obj.es_frontera, help="Afecta Comisión Conductor y Cruce Frontera")
                ...
                with col2:
                    ...
                    if flypass > 0: st.caption(f"💵 {formatear_numero(flypass)}")
                   
                    peajes_texto = st.text_input("Peajes (COP)", value="", placeholder="0")
                    peajes = limpiar_numero(peajes_texto)
                    if peajes > 0: st.caption(f"💵 {formatear_numero(peajes)}")
                with col3:
                    hotel_texto = st.text_input("Hotel (COP)", value="", placeholder="0")
                    hotel = limpiar_numero(hotel_texto)
                    if hotel > 0: st.caption(f"💵 {formatear_numero(hotel)}")
                   
                    comida_texto = st.text_input("Comida (COP)", value="", placeholder="0")
                    comida = limpiar_numero(comida_texto)
                    if comida > 0: st.caption(f"💵 {formatear_numero(comida)}")
               
                col1, col2 = st.columns(2)
                with col1:
                    cargue_texto = st.text_input("Cargue/Descargue (COP)", value="", placeholder="0")
                    cargue_descargue = limpiar_numero(cargue_texto)
                    if cargue_descargue > 0: st.caption(f"💵 {formatear_numero(cargue_descargue)}")
                with col2:
                    # Checkbox Anticipo Empresa
                    hubo_anticipo_empresa = st.checkbox("¿Hubo anticipo?", value=True, help="Marca si se entregó el 90% de anticipo a la empresa")
                    # Anticipo Manual
                    anticipo_texto = st.text_input("Anticipo (COP)", value="", placeholder="0", help="Anticipo entregado al conductor para gastos")
                    anticipo = limpiar_numero(anticipo_texto)
                    if anticipo > 0: st.caption(f"💵 {formatear_numero(anticipo)}")
               
                col1, col2 = st.columns(2)
                with col1:
                    otros_texto = st.text_input("Otros - Engrase, Lavada, Policía (COP)", value="", placeholder="0")
                    otros = limpiar_numero(otros_texto)
                    if otros > 0: st.caption(f"💵 {formatear_numero(otros)}")
                with col2: pass # Espacio vacio para alinear
                
                st.divider(); st.subheader("💰 Valor del Flete")
                valor_flete_texto = st.text_input("💵 Valor del Flete Cobrado al Cliente (COP)", value="", placeholder="Ejemplo: 5.000.000", help="¿Cuánto VAS A COBRAR o YA COBRASTE por este viaje?")
                valor_flete = limpiar_numero(valor_flete_texto)
                if valor_flete > 0: st.success(f"✅ Flete: ${formatear_numero(valor_flete)} COP")
               
                # Mostrar preview de cálculo
                if valor_flete > 0:
                    calc_preview = CalculadoraCostos(tractomula_obj, conductor_obj, ruta_obj, dias_viaje, es_frontera, es_urbano, hubo_parqueo, flypass, peajes, hotel, comida, cargue_descargue, otros, anticipo, hubo_anticipo_empresa, valor_flete, datos)
                    ...
                    with col2:
                        utilidad_color = "normal" if costos_preview['utilidad'] >= 0 else "inverse"
                        st.metric("Utilidad", f"${formatear_numero(costos_preview['utilidad'])}", delta=f"{costos_preview['rentabilidad']:.1f}%")
                    with col3: st.metric("Punto Equilibrio", f"${formatear_numero(costos_preview['punto_equilibrio'])}")
                    with col4: st.metric("Saldo (Legalización)", f"${formatear_numero(costos_preview['saldo_anticipo'])}")
                ...
                if calcular or guardar:
                    if valor_flete <= 0: st.error("⚠️ Debes ingresar el Valor del Flete para continuar")
                    else:
                        calculadora = CalculadoraCostos(tractomula_obj, conductor_obj, ruta_obj, dias_viaje, es_frontera, es_urbano, hubo_parqueo, flypass, peajes, hotel, comida, cargue_descargue, otros, anticipo, hubo_anticipo_empresa, valor_flete, datos)
                        st.session_state.calculadoras.append(calculadora)
                        if guardar:
                            viaje_id = db.guardar_viaje(calculadora, observaciones)
                            costos = calculadora.calcular_costos_totales()
                            if costos['utilidad'] >= 0:
                                st.success(f"""✅ **Viaje guardado exitosamente (ID: {viaje_id})**
- Total Gastos: ${formatear_numero(costos['total_gastos'])}
- Valor Flete: ${formatear_numero(valor_flete)}
- **Utilidad: ${formatear_numero(costos['utilidad'])}**
- **Rentabilidad: {costos['rentabilidad']:.1f}%**""")
                            else:
                                st.error(f"""⚠️ **Viaje guardado (ID: {viaje_id}) - PÉRDIDA DETECTADA**
- Total Gastos: ${formatear_numero(costos['total_gastos'])}
- Valor Flete: ${formatear_numero(valor_flete)}
- **Pérdida: ${formatear_numero(costos['utilidad'])}**
- **Rentabilidad: {costos['rentabilidad']:.1f}%**
⚠️ Este viaje NO fue rentable.""")
                        else: st.success("✅ Cálculo completado! Ve a la pestaña de Reportes.")
    ...
    with tab5:
        ...
        if st.session_state.calculadoras:
            ...
            ultimo = st.session_state.calculadoras[-1]; conductor_nombre = ultimo.conductor.nombre.strip(); placa = ultimo.tractomula.placa.strip()
            fecha_archivo = datetime.now().strftime('%d-%m-%Y')
            nombre_archivo = f"{conductor_nombre} {placa} {fecha_archivo}.xlsx"
            nombre_archivo = re.sub(r'[\\/:*?"<>|]', '-', nombre_archivo)
            st.download_button(label="📥 Descargar Reporte Completo en Excel", data=excel_data, file_name=nombre_archivo, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
           
            if st.button("🗑️ Limpiar reportes temporales"):
                st.session_state.calculadoras = []; st.rerun()
        else: st.info("Realiza al menos un cálculo en la pestaña anterior para ver reportes.")
    ...
    with tab6:
        ...
        if df_viajes.empty: st.info("No se encontraron viajes con los filtros aplicados.")
        else:
            ...
            with col3: st.metric("Total Gastos", f"${formatear_numero(df_viajes['total_gastos'].sum())}")
            with col4: st.metric("Total Utilidad", f"${formatear_numero(df_viajes['utilidad'].sum())}")
           
            st.subheader("Resultados")
            columnas_mostrar = ['id', 'fecha_creacion', 'placa', 'conductor', 'origen', 'destino', 'distancia_km', 'dias_viaje', 'total_gastos', 'anticipo', 'legalizacion', 'saldo_anticipo', 'valor_flete', 'utilidad', 'rentabilidad']
            df_mostrar = df_viajes[columnas_mostrar].copy()
            df_mostrar.columns = ['ID', 'Fecha', 'Placa', 'Conductor', 'Origen', 'Destino', 'Km', 'Días', 'Total Gastos', 'Anticipo', 'Legalización', 'Saldo (Ant-Leg)', 'Valor Flete', 'Utilidad', 'Rentabilidad %']
            for col in ['Total Gastos', 'Anticipo', 'Legalización', 'Saldo (Ant-Leg)', 'Valor Flete', 'Utilidad']:
            ...
            if st.button("Ver Detalle Completo"):
                ...
                if viaje:
                    ...
                    with col1:
                        ...
                        st.write(f"**ID:** {viaje[0]}"); st.write(f"**Fecha:** {viaje[1]}"); st.write(f"**Placa:** {viaje[2]}")
                        st.write(f"**Conductor:** {viaje[3]}"); st.write(f"**Ruta:** {viaje[4]} → {viaje[5]}")
                        st.write(f"**Distancia:** {formatear_numero(viaje[6])} km"); st.write(f"**Días:** {viaje[7]}")
                        st.write(f"**Es Frontera:** {'Sí' if viaje[8] else 'No'}"); st.write(f"**Es Urbano:** {'Sí' if viaje[9] else 'No'}") # Nuevo indice
                        st.write(f"**Hubo Parqueo:** {'Sí' if viaje[10] else 'No'}")
                    with col2:
                        st.markdown("### 💰 Resultados Financieros")
                        # Indices corregidos por nuevas columnas
                        st.write(f"**Total Gastos:** ${formatear_numero(viaje[29])}")
                        st.write(f"**Legalización:** ${formatear_numero(viaje[30])}")
                        st.write(f"**Anticipo:** ${formatear_numero(viaje[31])}") # Nuevo
                        st.write(f"**Saldo (Ant-Leg):** ${formatear_numero(viaje[32])}") # Nuevo
                        st.write(f"**Punto Equilibrio:** ${formatear_numero(viaje[33])}")
                        st.write(f"**Valor Flete:** ${formatear_numero(viaje[34])}")
                        utilidad = viaje[35]; rentabilidad = viaje[36]
                        if utilidad >= 0:
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.write(f"1. Nómina Admin: ${formatear_numero(viaje[11])}")
                        st.write(f"2. Nómina Conductor: ${formatear_numero(viaje[12])}")
                        st.write(f"3. Comisión Conductor: ${formatear_numero(viaje[13])}")
                        st.write(f"4. Mantenimiento: ${formatear_numero(viaje[14])}")
                        st.write(f"5. Seguros: ${formatear_numero(viaje[15])}")
                        st.write(f"6. Tecnomecánica: ${formatear_numero(viaje[16])}")
                    with col2:
                        st.write(f"7. Llantas: ${formatear_numero(viaje[17])}")
                        st.write(f"8. Aceite: ${formatear_numero(viaje[18])}")
                        st.write(f"9. Combustible: ${formatear_numero(viaje[19])}")
                        st.write(f" - Galones: {formatear_decimal(viaje[20])}")
                        st.write(f"10. Flypass: ${formatear_numero(viaje[21])}")
                        st.write(f"11. Peajes: ${formatear_numero(viaje[22])}")
                    with col3:
                        st.write(f"12. Cruce Frontera: ${formatear_numero(viaje[23])}")
                        st.write(f"13. Hotel: ${formatear_numero(viaje[24])}")
                        st.write(f"14. Comida: ${formatear_numero(viaje[25])}")
                        st.write(f"15. Parqueo: ${formatear_numero(viaje[26])}")
                        st.write(f"16. Cargue/Descargue: ${formatear_numero(viaje[27])}")
                        st.write(f"17. Otros: ${formatear_numero(viaje[28])}")
                    
                    if viaje[39]: # Observaciones ahora en 39
                        st.markdown("### 📝 Observaciones")
                        st.info(viaje[39])
                    if st.button("🗑️ Eliminar este viaje", type="secondary"):
                        db.eliminar_viaje(viaje_id_seleccionado); st.success("Viaje eliminado"); st.rerun()
            ...
            if st.button("Descargar en Excel"):
                ...
                st.download_button(label="📥 Descargar Historial en Excel", data=output, file_name=f"Historial_Viajes_{fecha_descarga}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        with col2:
            ...
            if stats['viajes_por_placa']:
                for placa, total in stats['viajes_por_placa'][:5]: st.write(f"**{placa}:** {total} viajes")
            else: st.info("No hay datos")
        with col3:
            st.markdown("### Top Conductores")
            if stats['viajes_por_conductor']:
                for conductor, total in stats['viajes_por_conductor'][:5]: st.write(f"**{conductor}:** {total} viajes")
            else: st.info("No hay datos")
    ...
    with tab7:
        ...
        with st.expander("🔍 Filtros por Fecha", expanded=True):
            ...
            if filtro_tipo == "Mes":
                ...
                col1, col2 = st.columns(2)
                with col1: fecha_inicio = st.date_input("Desde", value=None)
                with col2: fecha_fin = st.date_input("Hasta", value=None)
        ...
        if df_totales.empty: st.info("No hay datos con los filtros aplicados.")
        else:
            df_mostrar = df_totales.copy()
            for col in df_mostrar.columns:
                if col != 'placa' and col.startswith('total_'):
                    df_mostrar[col] = df_mostrar[col].apply(lambda x: f"${formatear_numero(x)}" if 'rentabilidad' not in col else f"{x:.1f}%")
            st.dataframe(df_mostrar, use_container_width=True)

            excel_totales = GeneradorReportes.generar_excel_totales(df_totales)
            st.download_button(label="📥 Descargar Reporte en Excel", data=excel_totales, file_name=f"Acumulados_Flota_{datetime.now().strftime('%Y-%m-%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

            st.subheader("Gráficos Comparativos")
            fig_cxc = px.bar(df_totales, x='placa', y='total_cxc', title="Total CXC por Unidad"); st.plotly_chart(fig_cxc)
            fig_gastos = px.bar(df_totales, x='placa', y='total_gastos', title="Total Gastos por Unidad"); st.plotly_chart(fig_gastos)
            fig_ut = px.bar(df_totales, x='placa', y='total_ut', title="Total UT por Unidad"); st.plotly_chart(fig_ut)
            fig_rentabilidad = px.bar(df_totales, x='placa', y='total_rentabilidad', title="Rentabilidad por Unidad"); st.plotly_chart(fig_rentabilidad)
