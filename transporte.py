...
class DatabaseManager:
    ...
    def guardar_viaje(self, calculadora, observaciones=""):
        ...
        cursor.execute('''
            INSERT INTO viajes (
                ...
                distancia_km, dias_viaje, es_frontera, hubo_parqueo,
    ...
    def obtener_totales_por_placa(self, fecha_inicio=None, fecha_fin=None):
        ...
        if not df.empty:
            ...
            df['saldo_total'] = df['total_ant_empresa'] - df['total_legalizacion']
    ...
    def guardar_tractomula(self, tractomula: 'Tractomula'):
        """Guarda una tractomula en la base de datos"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('''
    ...
    def obtener_tractomulas(self):
        ...
        for row in cursor.fetchall():
            tractomulas.append(Tractomula(
                placa=row[1], consumo_km_galon=row[2], tipo=row[3]
            ))
        conn.close()
        return tractomulas
    ...
    def obtener_rutas(self):
        ...
        for row in cursor.fetchall():
            rutas.append(Ruta(
                origen=row[1], destino=row[2], distancia_km=row[3], es_frontera=bool(row[4])
            ))
        conn.close()
        return rutas
...
class DatosColombia:
    ...
    # Constantes para fórmulas
    NOMINA_ADMIN_BASE = 808742
    NOMINA_ADMIN_DIVISOR = 14
    NOMINA_CONDUCTOR_DIA = 18278
    ...
    SEGURO_2 = 6000000
    SEGURO_3 = 16000000
    TECNOMECANICA_ANUAL = 400000
    LLANTAS_COSTO = 1200000
    LLANTAS_CANTIDAD = 22
    LLANTAS_KM = 80000
...
class CalculadoraCostos:
    ...
    def calcular_costos_totales(self) -> Dict[str, float]:
        ...
        # Anticipo empresa
        ant_empresa = self.valor_flete * self.datos.MARGEN_ANT_EMPRESA
...
"""
    ...
    def generar_excel(calculadoras: List[CalculadoraCostos]) -> io.BytesIO:
        ...
        headers = ['Ruta', 'Placa', 'Conductor', 'Distancia (km)', 'Días', 'Galones',
                   'Combustible', 'Total Gastos', 'Valor Flete', 'Utilidad',
        ...
        for calc in calculadoras:
            ...
            ruta = f"{calc.ruta.origen} → {calc.ruta.destino}"
...
def input_numero(label, value=0.0, min_value=0.0, step=1000.0, key=None, help=None):
    ...
    # Crear un placeholder único
    placeholder_key = f"placeholder_{key}" if key else None
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Estos valores están fijos según tus fórmulas")
        st.info(f"""
        **Nóminas:**
        - Admin base: ${formatear_numero(datos.NOMINA_ADMIN_BASE)}
        - Conductor/día: ${formatear_numero(datos.NOMINA_CONDUCTOR_DIA)}
       
        **Comisiones:**
        - Frontera: ${formatear_numero(datos.COMISION_FRONTERA)}
        - No frontera: ${formatear_numero(datos.COMISION_NO_FRONTERA)}
    ...
    with tab2:
        ...
        with st.form(key="form_ruta"):
            ...
            with col2:
                es_frontera = st.checkbox("¿Es ruta a frontera?", help="Activa si el destino es hacia frontera (Cúcuta, Ipiales, Maicao, etc.)")
            ...
            if submit and origen and destino:
                ...
                ruta = Ruta(origen, destino, distancia_km, es_frontera)
        ...
        if st.session_state.rutas:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT id, origen, destino, distancia_km, es_frontera FROM rutas ORDER BY origen, destino")
            rutas_con_id = cursor.fetchall()
            conn.close()
           
            for ruta_data in rutas_con_id:
                ruta_id, origen, destino, dist, es_front = ruta_data
                col1, col2 = st.columns([4, 1])
                with col1:
                    frontera_tag = "🌐 FRONTERA" if es_front else ""
                    st.write(f"**{origen}** → **{destino}** ({formatear_numero(dist)} km) {frontera_tag}")
                with col2:
            ...
            if st.button("Ver Detalle Completo"):
                ...
                if viaje:
                    ...
                    with col2:
                        ...
                        st.write(f"**Ant. Empresa (90%):** ${formatear_numero(viaje[34])}")
                        st.write(f"**Saldo Empresa:** ${formatear_numero(viaje[35])}")
